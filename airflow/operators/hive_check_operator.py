...
class HiveCheckOperator(CheckOperator):
    """
    Performs checks against Hive. The ``HiveCheckOperator`` expects
    ...
    * Empty dictionary or set (``{}``)
    ...
    def __init__(
            ...
            *args, **kwargs):
        super(HiveCheckOperator, self).__init__(sql=sql, *args, **kwargs)

        self.hive_conn_id = hive_conn_id
        self.sql = sql

    def get_db_hook(self):
        return HiveHook(hive_conn_id=self.hive_conn_id)
